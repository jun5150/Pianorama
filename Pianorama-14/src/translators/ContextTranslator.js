...
window.ContextTranslator = {
    ...
    GLYPHS: { sharp: "\uE262", flat: "\uE260", natural: "\uE261", doubleSharp: "\uE263", doubleFlat: "\uE264" },
    currentKey: "C",
    ...
    setContext: function(key) {
        if (!key) return;
        this.currentKey = key;
        var flats = ["F", "Bb", "Eb", "Ab", "Db", "Gb", "Cb", "f", "bb", "eb", "ab", "db", "gb"];
    ...
    translate: function(midi, targetLetter) {
        ...
        var info;
        // Se houver targetLetter, forçamos a grafia (ex: C D Eb em vez de C D D#)
        if (targetLetter) {
            info = this._getNoteByLetter(pc, targetLetter.toUpperCase());
        } else {
            info = this._inferNoteByContext(pc);
        }

        // Ajuste de oitava visual para notas como B#4 (desenhar na linha do B4)
    ...
    _getNoteByLetter: function(pc, targetLetter) {
        ...
        // Ajuste para distâncias em círculo
        if (diff > 6) diff -= 12;
        if (diff < -6) diff += 12;
    ...
    _getFileName: function(l, a, o) {
        var name = l + a;
        var map = { "Db":"C#","Eb":"D#","Gb":"F#","Ab":"G#","Bb":"A#","Cb":"B","Fb":"E","E#":"F","B#":"C" };
        return (map[name] || name) + o + ".mp3";
    },

    _getGlyph: function(acc) {
        var map = { "#":this.GLYPHS.sharp, "b":this.GLYPHS.flat, "##":this.GLYPHS.doubleSharp, "bb":this.GLYPHS.doubleFlat };
        return map[acc] || "";
    }
};
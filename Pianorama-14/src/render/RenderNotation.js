/**
 * PIANORAMA - RenderNotation.js (v13.0)
 */
window.RenderNotation = {
    ...
    _drawNoteHead: function(ctx, x, yBase, noteObj, config) {
        ...
        // LÓGICA DE EXIBIÇÃO DE ACIDENTES (Pro vs Beginner)
        var mode = config.accidentalMode || "both"; // both = Modo Iniciante
        var showAccidental = false;

        if (noteObj.accidental) {
            if (mode === "both" || mode === "notes") {
                showAccidental = true; // Sempre mostra
            } else if (mode === "signature") {
                // Modo Pro: Só mostra se a nota NÃO estiver na armadura de clave
